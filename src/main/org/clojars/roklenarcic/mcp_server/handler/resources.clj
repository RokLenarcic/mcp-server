...
(defn resources-list
  ...
  [rpc-session req-meta {:keys [cursor] :as params}]
  ...
  (if-let [res (resources' rpc-session)]
    (do
      ...
      (res/list-resources res (common/create-req-session rpc-session req-meta params) cursor))
    (do
...
(defn wrap-resource
  ...
  [handler]
  (fn [rpc-session req-meta {:keys [uri] :as params}]
    (log/trace "Processing resource request for URI:" uri)
    
    (if-let [resources (resources' rpc-session)]
      (if (string? uri)
        (let [exchange (common/create-req-session rpc-session req-meta params)]
...
(defn notify-changed
  ...
  (let [resources (resources' rpc-session)
        exchange (common/create-req-session rpc-session nil nil)]
